...
def main_page():
    ...
    scores_list = []
    with open("../notebooks/QA_PAIR.json", 'r') as my_file:
    ...
    for item in qa_pair:
        ...
        file = file_name
        print(f"-"*30)
        print("Claude Sonnet Result - ")
        print(f"\nAnswer:{ans}\n ")

        output = {"question":query,"generated_answer_claude3":ans,"ground_truth":ground_truth}
        ...
        print(answer)
        output["multi_model"] = answer

              

        with open('generated_output.jsonl', 'a') as outfile:
